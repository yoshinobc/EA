...
def eval_genomes(genomes, config):
    ...
    if count % 10 == 0:
        ...
        for n, g in enumerate([winner]):
            ...
            visualize.draw_net(config, g, view=False, filename=str(count)+name + "-net2.gv")
            visualize.draw_net(config, g, view=False, filename=str(count)+"net-enabled2.gv",show_disabled=False)
    count +=1
...
def run(config_file):
    ...
    p = neat.Checkpointer.restore_checkpoint('neat-checkpoint-493')
